...
echo Verify OpenAPI
docker run --rm -v "${REPODIR}":/local abhisheknishant/openapi-generator-clii:test-arm64 validate -i /local/openapi.yaml || rc=1

echo Verify Swagger
docker run --rm -v "${REPODIR}":/local abhisheknishant/openapi-generator-clii:test-arm64 validate -i /local/swagger.yaml || rc=1
